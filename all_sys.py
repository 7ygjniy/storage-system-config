...
import numpy as np # Global import for GBR engine
...
def calculate_area_small_project(system_type, n_sets, system_duration_h):
    """
    计算小型项目（体量 < 50MWh）的储能区占地面积（不含环形道路）
    
    参数:
        system_type: 系统类型 ("5MW" 或 "7.5MW")
        n_sets: 设备套数
        system_duration_h: 系统时长（小时）
    ...
    if system_type == "5MW":
        # F_5(N,T) 公式
        time_factor = 1 + (T - 2) * (0.008274 * N**2 - 0.03836 * N + 0.3798)
        base_area = (0.29048 * N**4 - 3.02368 * N**3 + 9.85548 * N**2 + 157.42735 * N - 33.5255)
        area = time_factor * base_area
    else:  # 7.5MW
        # F_7.5(N,T) 公式
        time_factor = 1 + (T - 2) * (-0.00758168 * N + 0.50613656)
        base_area = (1.39875 * N**2 + 218.533036 * N - 48.375)
        area = time_factor * base_area
...
class TreeEstimator:
    def __init__(self, tree_struct):
        self.children_left = tree_struct['children_left']
        self.children_right = tree_struct['children_right']
        self.feature = tree_struct['feature']
        self.threshold = tree_struct['threshold']
        self.value = tree_struct['value']
    
    def predict_single(self, sample):
        node = 0
        # 循环直到叶子节点（叶子节点的 children_left 为 -1）
        while self.children_left[node] != -1:
            feature_idx = self.feature[node]
            # 严格使用 <= 判断，与 sklearn 一致
            if sample[feature_idx] <= self.threshold[node]:
                node = self.children_left[node]
            else:
                node = self.children_right[node]
        return self.value[node]

class GradientBoostingRegressorEngine:
    def __init__(self, model_struct):
        self.learning_rate = model_struct['learning_rate']
        self.init_constant = model_struct['init_constant']
        self.estimators = [TreeEstimator(tree) for tree in model_struct['trees']]
        
    def predict(self, X):
        # X shape: (n_samples, n_features)
        n_samples = X.shape[0]
        predictions = np.full(n_samples, self.init_constant)
        
        for i in range(n_samples):
            sample = X[i]
            tree_preds = 0.0
            for tree in self.estimators:
                tree_preds += tree.predict_single(sample)
            predictions[i] += self.learning_rate * tree_preds
            
        return predictions

def predict_area_large_project(project_capacity_mw, system_duration_h, transformer_count, system_type):
    """
    使用完全复刻的 GBR 模型预测大型项目（体量 >= 50MWh）的占地面积
    精度无损，且无需 scikit-learn
    
    参数:
        project_capacity_mw: 项目体量（MW）
        system_duration_h: 系统时长（2或4小时）
        transformer_count: 主变个数（1或2台）
        system_type: 系统类型（'5MW'或'7.5MW'）
    ...
    # 验证输入
    if system_type not in ['5MW', '7.5MW']:
        return {"error": "系统类型必须是 '5MW' 或 '7.5MW'"}
    
    if system_duration_h not in [2, 4]:
        return {"error": "系统时长必须是 2 或 4 小时"}
    
    if transformer_count not in [1, 2]:
        return {"error": "主变个数必须是 1 或 2"}
    
    # 加载模型结构 (JSON)
    model_file = './model_gbr_structure.json'
    try:
        with open(model_file, 'r', encoding='utf-8') as f:
            all_models = json.load(f)
    except FileNotFoundError:
        return {"error": f"模型结构文件 {model_file} 未找到"}
    
    if system_type not in all_models:
        return {"error": f"参数文件中未找到 {system_type} 的模型数据"}
        
    model_data = all_models[system_type]
    system_capacity = model_data['system_capacity']
    
    # --- 1. 预测储能区 ---
    # 特征: [mw, duration, mwh, n_sets]
    n_sets = project_capacity_mw / system_capacity
    capacity_mwh = project_capacity_mw * system_duration_h
    
    X_storage = np.array([[project_capacity_mw, system_duration_h, capacity_mwh, n_sets]])
    
    # Standard Scaler
    scaler_storage = StandardScaler(model_data['scaler_storage'])
    X_storage_scaled = scaler_storage.transform(X_storage)
    
    # GBR Predict
    gbr_storage = GradientBoostingRegressorEngine(model_data['model_storage'])
    storage_area = gbr_storage.predict(X_storage_scaled)[0]
    
    # --- 2. 预测升压站 ---
    # 特征: [mw, transformer_count]
    X_sub = np.array([[project_capacity_mw, transformer_count]])
    
    # Standard Scaler
    scaler_sub = StandardScaler(model_data['scaler_substation'])
    X_sub_scaled = scaler_sub.transform(X_sub)
    
    # GBR Predict
    gbr_sub = GradientBoostingRegressorEngine(model_data['model_substation'])
    substation_area = gbr_sub.predict(X_sub_scaled)[0]
    
    # 确保面积不为负数
    storage_area = max(0, storage_area)
    substation_area = max(0, substation_area)
    
    # 计算总面积
    total_area = storage_area + substation_area
    
    # 转换成亩（1亩 = 666.67 m²）
    storage_area_mu = storage_area / 666.67
    substation_area_mu = substation_area / 666.67
    total_area_mu = total_area / 666.67
    
    # 计算占比
    if total_area > 0:
        storage_ratio = (storage_area / total_area) * 100
        substation_ratio = (substation_area / total_area) * 100
    else:
        storage_ratio = 0
        substation_ratio = 0
    ...
    result = {
        ...
        "substation_area_m2": round(substation_area, 2),
        "total_area_m2": round(total_area, 2),
        "storage_area_mu": round(storage_area_mu, 2),
        "substation_area_mu": round(substation_area_mu, 2),
        "total_area_mu": round(total_area_mu, 2),
        "storage_ratio": round(storage_ratio, 1),
        "substation_ratio": round(substation_ratio, 1)
    }
    
    return result


def predict_land_area(project_power_mw, project_capacity_mwh, transformer_count=1, system_type='7.5MW'):
    """
    预测储能项目占地面积的统一入口函数
    
    参数:
        project_power_mw: 项目功率（MW）
        project_capacity_mwh: 项目容量（MWh）
        transformer_count: 主变个数（1或2台），默认1台
        system_type: 系统类型（'5MW'或'7.5MW'），默认'7.5MW'
    ...
    rounded_duration = 0
    if abs(duration_hours - 2) < 0.5:
        rounded_duration = 2
        is_standard_duration = True
    elif abs(duration_hours - 4) < 0.5:
        rounded_duration = 4
        is_standard_duration = True
    
    # 如果不是标准时长，返回提示
    if not is_standard_duration:
        return {
            "error": "non_standard_duration",
            "message": f"当前系统时长为 {duration_hours:.2f} 小时，占地面积预测仅支持 2h 或 4h 系统",
            "duration_hours": round(duration_hours, 2)
        }
    
    # 判断项目体量
    if project_capacity_mwh < 50:
        # 小型项目，使用数学公式
        # 计算设备套数
        system_capacity = 5.0 if system_type == "5MW" else 7.5
        n_sets = project_power_mw / system_capacity
        
        # 计算储能区面积（不含环形道路）
        storage_area = calculate_area_small_project(system_type, n_sets, rounded_duration)
        storage_area_mu = storage_area / 666.67
        ...
        result = {
            ...
            "storage_area_m2": round(storage_area, 2),
            "storage_area_mu": round(storage_area_mu, 2),
            "n_sets": round(n_sets, 2),
            "duration_hours": rounded_duration,
            "capacity_mwh": project_capacity_mwh,
            "note": "储能区占地面积（不含环形道路）"
        }
    else:
        # 大型项目，使用机器学习模型
        result = predict_area_large_project(
            project_power_mw, 
            rounded_duration, 
            transformer_count, 
            system_type
        ...
        if "error" not in result:
            result["duration_hours"] = rounded_duration
            result["capacity_mwh"] = project_capacity_mwh
